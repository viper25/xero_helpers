...
def align_columns_to_pledge_plg_table(df):
    ...
    crm_pledge_plg_db_columns = ['plg_FamID', 'plg_FYID', 'plg_date', 'plg_amount', 'plg_schedule', 'plg_method',
                                 ...
                                 'plg_fundID', 'plg_depID',
                                 'plg_CheckNo', 'plg_Problem', 'plg_scanString', 'plg_aut_ID', 'plg_aut_Cleared',
                                 'plg_aut_ResultID',
                                 'plg_NonDeductible', 'plg_GroupKey']
    ...
    df["plg_FYID"] = datetime.now().year - 1996
    # today's date
    df["plg_date"] = datetime.now().strftime("%Y-%m-%d")
    ...
    df["plg_EditedBy"] = 1082
    df["plg_PledgeOrPayment"] = "Payment"
    df["plg_fundID"] = df["Account"].apply(get_crm_fund_ID)
    df["plg_depID"] = 1
    df_merged["plg_CheckNo"] = 0
    # df_merged["plg_Problem"] = ""
    # df_merged["plg_scanString"] = ""
    df_merged["plg_aut_ID"] = 0
    df_merged["plg_aut_Cleared"] = 0
    df_merged["plg_aut_ResultID"] = 0
    # df_merged["plg_NonDeductible"] = ""
    # Concatenate plg_FamID, plg_fundID,plg_date  to create GroupKey
    df["plg_GroupKey"] = f"'cash'|0|{df['plg_FamID']}|{df['plg_fundID']}|{df['plg_date']}"
