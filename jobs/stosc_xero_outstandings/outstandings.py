...
while has_more_pages:
    page += 1
    invoices = utils.xero_get(
        f"https://api.xero.com/api.xro/2.0/Invoices?Statuses=AUTHORISED,PAID&where=Type%3D%22ACCREC%22&summaryOnly=true&page={page}")
...
df_reset['year'] = df_reset.iloc[:, 0].str.extract(r'(\d+)').astype(int)
